module GitWakaTime
  ...
  class Controller
    def initialize(path: '.', date: nil)
      ...
      @relevant_commits = Commit.where('date > ? and project = ?', date, @project)

      @files = CommitedFile.where('commit_id IN ?',  @relevant_commits.select_map(:id) ).where('project = ?', @project)

      @actions = Query.new(@relevant_commits,@files, File.basename(path)).get
    end

    def timer
      Timer.new(@relevant_commits.all, @actions, File.basename(@path)).process
    end
  end
end
